...
struct vector *vecSum(struct vector *vec1, struct vector *vec2){
    ...
    if (sum == NULL || head_cord_sum == NULL) {
        printf("An Error Has Occurred\n");
        exit(1);
    }
...
int main(int argc, char *argv[]){
    ...
    for(i=0; i<k; i++){
        ...
        if (curr_centroid->next == NULL) {
                printf("4An Error Has Occurred\n");
                freeVectors(head_vec);
                freeVectors(head_centroid);
                return 1;
            }
    ...
    for(i=0; i<iter; i++){
        ...
        if (potential_head_centroid == NULL) {
            printf("An Error Has Occurred\n");
            freeVectors(head_vec);
            freeVectors(head_centroid);
            return 1;
        ...
        if (partition == NULL) {
            ...
            freeVectors(head_vec);
            freeVectors(head_centroid);
            freeVectors(potential_head_centroid);
            return 1;
        }
        ...
        for(j=0; j<k; j++){
            ...
            if (j < k-1){
                ...
                if (potential_curr_centroid->next == NULL) {
                    printf("An Error Has Occurred\n");
                    freeVectors(head_vec);
                    freeVectors(head_centroid);
                    freeVectors(potential_head_centroid);
